...
def _seed_baraban_prizes(conn):
    """Boshlang'ich placeholder sovg'alar katalogi. Keyinchalik narx/nom/
    ehtimollik ('weight') admin panelidan yoki shu jadvalni to'g'ridan-to'g'ri
    yangilash orqali osongina o'zgartiriladi. ON CONFLICT DO NOTHING - qayta
    ishga tushirilganda mavjud qatorlar ustidan yozib yubormaydi."""
    prizes = [
        ("baraban_yutuqsiz", "Yutuqsiz",     "💨", 40, 1),
        ("baraban_uc60",     "60 UC",        "🪙", 20, 2),
        ("baraban_uc120",    "120 UC",       "🪙", 12, 3),
        ("baraban_stars50",  "50 Stars",     "⭐", 15, 4),
        ("baraban_stars100", "100 Stars",    "🌟", 8,  5),
        ("baraban_bonus10k", "10 000 so'm",  "💰", 4,  6),
        ("baraban_premium",  "Premium 1 kun","💎", 1,  7),
    ]
    for pid, name, icon, weight, order in prizes:
        conn.execute(
            "INSERT INTO baraban_prizes (id, name, icon, weight, sort_order) "
            "VALUES (?,?,?,?,?) ON CONFLICT (id) DO NOTHING",
