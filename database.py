...
def init_db():
    ...
    # "Omad barabani" — chipta bilan aylantiriladigan sovg'a baraban katalogi
    conn.execute("""
        CREATE TABLE IF NOT EXISTS baraban_prizes (
            id          TEXT PRIMARY KEY,
            name        TEXT NOT NULL,
            icon        TEXT,
    ...
    # "Omad barabani" — aylantirishlar tarixi (so'nggi g'oliblar ro'yxati uchun)
    conn.execute("""
    ...
    # "Omad barabani" — avtomatik qo'llaniladigan voucherlar (masalan UC yoki
    # VIP chegirma). Foydalanuvchi barabanni aylantirib shu turdagi sovg'a
    # yutganda bu yerga bir martalik voucher yoziladi, keyin u UC sotib olish
    # yoki VIP xarid bo'limida qo'llanilib "ishlatildi" deb belgilanadi.
    conn.execute("""
        CREATE TABLE IF NOT EXISTS baraban_vouchers (
            id          BIGSERIAL PRIMARY KEY,
            user_id     BIGINT NOT NULL,
            kind        TEXT NOT NULL,      -- 'uc' | 'discount'
            value       INTEGER NOT NULL,   -- 'uc' uchun UC miqdori, 'discount' uchun foiz
            spin_id     BIGINT,
    ...
    if not _column_exists(conn, "users", "tickets"):
        conn.execute("ALTER TABLE users ADD COLUMN tickets INTEGER DEFAULT 0")
    if not _column_exists(conn, "baraban_spins", "delivered"):
        conn.execute("ALTER TABLE baraban_spins ADD COLUMN delivered INTEGER DEFAULT 0")
    ...
    # "Omad barabani" katalogini bir martalik urug'lantiramiz - hozircha
    # placeholder sovg'alar, keyinchalik admin panelidan o'zgartiriladi.
    _seed_baraban_prizes(conn)
...
def approve_uc_order(order_id: int) -> bool:
    """UC buyurtmasini tasdiqlaydi. Faqat 'pending' holatdagi buyurtmani
    tasdiqlaydi — shu tufayli bir xil buyurtma ham botdan, ham admin
    panelidan bir vaqtda tasdiqlansa ham, ikkinchi urinish xavfsiz
    rad etiladi (False qaytaradi)."""
    conn = get_conn()
    row = conn.execute(
        "SELECT status FROM uc_orders WHERE id=?", (order_id,)
    ).fetchone()
    if not row or row["status"] != "pending":
        conn.close()
        return False
...
def join_wheel_room(user_id: int, first_name: str, bet_amount: int) -> dict:
    ...
    conn = get_conn()
    try:
        # XAVFSIZLIK: oldin "already_in_room" tekshiruvi pul miqdori uchun
        # advisory lock olishdan OLDIN bajarilardi. Shu sababli bir xil
        # foydalanuvchidan ikkita parallel so'rov (masalan tugmani tez-tez
        # bosish) ikkalasi ham "xonada emas" deb ko'rib, ikkalasi ham
        # qo'shilishga urinishi va wheel_players jadvalidagi UNIQUE
        # cheklovga urilib 500-xato qaytarishi mumkin edi (pul yo'qolmaydi,
        # lekin foydalanuvchiga tushunarsiz xato ko'rinardi). Buning oldini
        # olish uchun avval shu FOYDALANUVCHI bo'yicha advisory lock olamiz —
        # shu bilan bir xil user_id uchun barcha parallel so'rovlar navbat
        # bilan ishlanadi va "already_in_room" tekshiruvi endi ishonchli.
...
def get_wheel_room(room_id: int, user_id: int = None):
    ...
    # Premium tugagan o'yinchining eski avatar rasmi g'ildirak xonasida
    # ham endi ko'rsatilmaydi.
    players_out = []
    for p in players:
        p_dict = dict(p)
        if not is_premium_active(p_dict["user_id"]):
            p_dict["avatar_url"] = None
        players_out.append(p_dict)
...
# ---------------------------------------------------------------
# "Omad barabani" — chipta bilan aylantiriladigan sovg'a baraban
# ---------------------------------------------------------------

def _seed_baraban_prizes(conn):
    """'Omad barabani' sovg'alar katalogi. 'weight' ustuni ehtimollikni
    bildiradi (kattaroq son = ko'proq chiqadi). Bu funksiya har safar
    serverdan chaqirilganda ro'yxatni shu yerdagi qiymatlar bilan
    sinxronlaydi (ON CONFLICT DO UPDATE) - shuning uchun narx/ehtimollikni
    o'zgartirish uchun shunchaki shu ro'yxatni tahrirlab qayta deploy
    qilish kifoya. Ro'yxatda endi yo'q eski yozuvlar avtomatik
    faolsizlantiriladi (active=0)."""
    # DIQQAT (2026-08-18 tuzatildi, keyin 2026-08-18 qayta ko'rib chiqildi):
    # Birinchi tuzatishda premium/chegirma narxlarini webapp_api.py dagi ESKI
    # VIP_PACKAGES asosida hisoblab (15kun=30000, 1oy=60000, 1yil=500000 so'm)
    # ularni haddan tashqari siyrak qilib qo'ygan edik. Amalda ilovadagi haqiqiy
    # narxlar ancha past: 15kun=5000, 1oy=10000, 3oy=20000, 1yil=50000 so'm.
    # Shu sababli premium/chegirma sovg'alari aslida juda arzon (jami EVning
    # atigi ~1.9%i) - shuning uchun ularning weight'ini oshirdik, foydalanuvchi
    # tez-tez kichik yutuq ko'rishi uchun. UC va naqd so'm EVning ~98%ini
    # tashkil qiladi (~51% UC, ~47% naqd) - bular asosiy zarar manbai bo'lib
    # qoladi, shuning uchun ularning weight'i o'zgarmadi.
    # Umumiy weight yig'indisi 100000 - foiz sifatida o'qish oson.
    prizes = [
        ("baraban_som0",     "0 so'm",             "💨", 64200, 1),
        ("baraban_som50",    "50 so'm",            "💰", 11500, 2),
        ("baraban_som100",   "100 so'm",           "💰", 8600,  3),
        ("baraban_som1000",  "1000 so'm",          "💰", 5750,  4),
        ("baraban_uc180",    "180 UC",             "🪙", 350,   5),
        ("baraban_disc10",   "10% VIP chegirma",   "🏷️", 1500,  6),
        ("baraban_som5000",  "5000 so'm",          "💰", 2900,  7),
        ("baraban_uc60",     "60 UC",              "🪙", 1150,  8),
        ("baraban_disc25",   "25% VIP chegirma",   "🏷️", 700,   9),
        ("baraban_som10000", "10000 so'm",         "💰", 1700,  10),
        ("baraban_uc120",    "120 UC",             "🪙", 700,   11),
        ("baraban_prem15",   "15 kunlik premium",  "💎", 500,   12),
        ("baraban_disc50",   "50% VIP chegirma",   "🏷️", 200,   13),
        ("baraban_prem1m",   "1 oylik premium",    "💎", 200,   14),
        ("baraban_elite",    "Elite Pass",         "🎫", 2,     15),
        ("baraban_prem1y",   "1 yillik premium",   "💎", 48,    16),
    ]
    ids = [p[0] for p in prizes]
    for pid, name, icon, weight, order in prizes:
        conn.execute(
            "INSERT INTO baraban_prizes (id, name, icon, weight, sort_order, active) "
            "VALUES (?,?,?,?,?,1) "
            "ON CONFLICT (id) DO UPDATE SET name=EXCLUDED.name, icon=EXCLUDED.icon, "
            "weight=EXCLUDED.weight, sort_order=EXCLUDED.sort_order, active=1",
            (pid, name, icon, weight, order)
        )
    # Ro'yxatda endi yo'q eski placeholder yozuvlarni faolsizlantiramiz
    placeholders = ",".join(["?"] * len(ids))
    conn.execute(
        f"UPDATE baraban_prizes SET active=0 WHERE id NOT IN ({placeholders})",
        tuple(ids)
    )
    conn.commit()
...
def add_tickets(user_id: int, amount: int) -> int:
    """Foydalanuvchiga chipta qo'shadi (yoki manfiy son bilan ayiradi).
    Yangi umumiy chipta sonini qaytaradi. Admin panel orqali qo'lda
    chipta berish uchun ishlatiladi."""
    conn = get_conn()
    conn.execute(
        "UPDATE users SET tickets = GREATEST(0, COALESCE(tickets,0) + ?) WHERE user_id=?",
        (amount, user_id)
    )
    conn.commit()
    row = conn.execute("SELECT tickets FROM users WHERE user_id=?", (user_id,)).fetchone()
...
def buy_tickets(user_id: int, count: int, price: int) -> dict:
    """Balansdan pul yechib, chipta sotib oladi. Narx chaqiruvchi tomonda
    (webapp_api.py, TICKET_PACKAGES orqali) allaqachon tekshirilgan bo'lishi
    kerak - bu funksiya faqat balans yetarli/yetarli emasligini tekshiradi."""
    conn = get_conn()
    try:
        row = conn.execute(
            "SELECT balance FROM users WHERE user_id=? FOR UPDATE", (user_id,)
        ).fetchone()
        if not row or (row["balance"] or 0) < price:
            conn.rollback()
            conn.close()
            return {"error": "insufficient_balance"}

        conn.execute(
            "UPDATE users SET balance = balance - ?, tickets = COALESCE(tickets,0) + ? WHERE user_id=?",
            (price, count, user_id)
        )
        conn.commit()

        final = conn.execute(
            "SELECT balance, tickets FROM users WHERE user_id=?", (user_id,)
        ).fetchone()
        conn.close()
        return {"ok": True, "balance": final["balance"], "tickets": final["tickets"]}
    except Exception:
        conn.rollback()
        conn.close()
        raise
...
def get_baraban_catalog() -> list:
    conn = get_conn()
    rows = conn.execute(
        "SELECT id, name, icon, weight FROM baraban_prizes "
...
def get_baraban_recent_winners(limit: int = 20) -> list:
    conn = get_conn()
    rows = conn.execute(
        "SELECT bs.user_id, bs.first_name, bs.prize_id, bs.prize_name, bs.prize_icon, bs.created_at, "
        "u.avatar_url "
        "FROM baraban_spins bs LEFT JOIN users u ON u.user_id = bs.user_id "
        "ORDER BY bs.created_at DESC LIMIT ?",
        (limit,)
    ).fetchall()
    conn.close()

    # Faqat premium muddati hali tugamagan foydalanuvchilarga avatarka va
    # premium belgisi ko'rsatiladi - "top o'yinchilar" bo'limidagi bilan bir xil
    # qoida (premium tugagan bo'lsa eski avatar va belgi ko'rinmasin).
    winners_out = []
    for r in rows:
        w = dict(r)
        premium_active = is_premium_active(w["user_id"])
        w["is_premium"] = premium_active
        if not premium_active:
            w["avatar_url"] = None
        winners_out.append(w)

    return winners_out


def get_baraban_deliveries(pending_only: bool = True, limit: int = 100) -> list:
    """Admin panel uchun - 'Baraban yutuqlari' ro'yxati. Pul (so'm)
    mukofotlari avtomatik berilgani uchun bu yerda odatda ko'rinmaydi
    (delivered=1 bilan yozilgan) - faqat UC/Premium/Elite Pass/chegirma
    kabi qo'lda berilishi kerak bo'lganlar chiqadi."""
    conn = get_conn()
    if pending_only:
        rows = conn.execute(
            "SELECT bs.id, bs.user_id, bs.first_name, bs.prize_id, bs.prize_name, "
            "bs.prize_icon, bs.delivered, bs.created_at, u.username "
            "FROM baraban_spins bs LEFT JOIN users u ON u.user_id = bs.user_id "
            "WHERE bs.delivered = 0 ORDER BY bs.created_at DESC LIMIT ?",
            (limit,)
        ).fetchall()
    else:
        rows = conn.execute(
...
def mark_baraban_delivered(spin_id: int) -> bool:
    conn = get_conn()
    conn.execute("UPDATE baraban_spins SET delivered=1 WHERE id=?", (spin_id,))
    conn.commit()
    row = conn.execute("SELECT id FROM baraban_spins WHERE id=?", (spin_id,)).fetchone()
    conn.close()
    return row is not None


# "Omad barabani" sovg'a nomidagi premium muddatini PREMIUM_DURATIONS
# kalitlariga moslashtiradi (masalan "baraban_prem15" -> "15kun").
_BARABAN_PREMIUM_KEY = {
    "baraban_prem15": "15kun",
    "baraban_prem1m": "1oy",
    "baraban_prem1y": "1yil",
}

# "Elite Pass" yutuqlari endi qo'lda admin tomonidan berilmaydi - u aslida
# "UC xarid qilish" bo'limidagi eng katta 325 UC paketiga teng, shuning
# uchun yutilganda avtomatik 325 UC voucher sifatida beriladi (xuddi
# boshqa "UC" sovg'alari kabi).
_BARABAN_ELITE_UC_VALUE = 325


def get_active_baraban_vouchers(user_id: int, kind: str = None) -> list:
    """Foydalanuvchining hali ishlatilmagan (used=0) barcha voucherlari.
    'kind' berilsa faqat shu turdagilar (masalan faqat 'uc')."""
    conn = get_conn()
    if kind:
        rows = conn.execute(
            "SELECT id, kind, value, spin_id, created_at FROM baraban_vouchers "
            "WHERE user_id=? AND used=0 AND kind=? ORDER BY created_at ASC",
            (user_id, kind)
        ).fetchall()
    else:
        rows = conn.execute(
            "SELECT id, kind, value, spin_id, created_at FROM baraban_vouchers "
            "WHERE user_id=? AND used=0 ORDER BY created_at ASC",
            (user_id,)
        ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
...
def get_baraban_voucher(voucher_id: int, user_id: int, kind: str = None):
    """Berilgan foydalanuvchiga tegishli, hali ishlatilmagan bitta voucherni
    qaytaradi (topilmasa None)."""
    conn = get_conn()
    if kind:
        row = conn.execute(
            "SELECT id, kind, value, spin_id, used FROM baraban_vouchers "
            "WHERE id=? AND user_id=? AND used=0 AND kind=?",
            (voucher_id, user_id, kind)
        ).fetchone()
    else:
        row = conn.execute(
            "SELECT id, kind, value, spin_id, used FROM baraban_vouchers "
...
def get_best_discount_voucher(user_id: int):
    """Foydalanuvchining hali ishlatilmagan barcha 'discount' voucherlari
    orasidan ENG KATTA foizlisini qaytaradi (yoki hech biri bo'lmasa None).
    Xarid vaqtida qaysi voucher_id mijoz tomonidan yuborilganidan qat'i
    nazar, har doim shu funksiya - ya'ni eng yuqori chegirma - haqiqiy
    manba (source of truth) hisoblanadi. Shu bilan bir nechta chegirma
    voucheri yig'ilib qolsa ham, faqat eng balandi qo'llanadi va
    pastroqlari uni bekor qilib yubormaydi."""
    conn = get_conn()
    row = conn.execute(
        "SELECT id, kind, value, spin_id, created_at FROM baraban_vouchers "
        "WHERE user_id=? AND used=0 AND kind='discount' "
...
def mark_baraban_voucher_used(voucher_id: int) -> bool:
    conn = get_conn()
    conn.execute(
        "UPDATE baraban_vouchers SET used=1, used_at=extract(epoch from now())::bigint WHERE id=?",
        (voucher_id,)
    )
    conn.commit()
    row = conn.execute("SELECT id FROM baraban_vouchers WHERE id=?", (voucher_id,)).fetchone()
    conn.close()
    return row is not None


def spin_baraban(user_id: int, first_name: str = None) -> dict:
    """1 ta chiptani sarflab barabanni bir marta aylantiradi. Sovg'a
    'weight' ustuniga proporsional ehtimollik bilan tanlanadi. Yutuq turiga
    qarab AVTOMATIK qo'llaniladi:
      - "so'm"    -> to'g'ridan-to'g'ri balansga qo'shiladi
      - "UC"      -> bir martalik UC voucher yaratiladi (UC sotib olish
                     bo'limida shu miqdorni 0 so'mga olish mumkin)
      - "chegirma"-> bir martalik VIP/Premium chegirma voucheri yaratiladi
                     (bir nechtasi bo'lsa ham faqat eng balandi qo'llanadi)
      - "premium" -> profilga zudlik bilan qo'shiladi (qo'shilmaydi -
                     faqat eng katta muddat amal qiladi, xuddi chegirma
                     kabi)
      - "Elite Pass" -> 325 UC qiymatidagi bepul UC voucheri sifatida
                     avtomatik beriladi (325 UC = "UC xarid qilish"
                     bo'limidagi eng katta paket narxiga teng)
    """
    conn = get_conn()
    try:
        row = conn.execute(
            "SELECT tickets FROM users WHERE user_id=? FOR UPDATE", (user_id,)
        ).fetchone()
        if not row or (row["tickets"] or 0) < 1:
            conn.rollback()
            conn.close()
            return {"error": "no_tickets"}

        prizes = conn.execute(
            "SELECT id, name, icon, weight FROM baraban_prizes WHERE active=1"
        ).fetchall()
        if not prizes:
            conn.rollback()
            conn.close()
        ...
        conn.execute(
            "UPDATE users SET tickets = tickets - 1 WHERE user_id=?", (user_id,)
        )

        digits = "".join(ch for ch in prize_name if ch.isdigit())
        is_cash = "so'm" in prize_name
        is_uc = prize_id.startswith("baraban_uc")
        is_elite = prize_id == "baraban_elite"
        is_discount = prize_id.startswith("baraban_disc")
        is_premium = prize_id in _BARABAN_PREMIUM_KEY
        delivered = 0

        if is_cash:
            # "0 so'm" yutuqda ham amal bajariladi (0 qo'shiladi), lekin
            # asosiysi - har qanday raqamli summa balansga to'g'ridan-to'g'ri
            # va zudlik bilan qo'shiladi.
            ...
            # Premium darhol profilga qo'shiladi - qo'shimcha qo'lda
            # tasdiqlash shart emas. DIQQAT: chegirma voucherlari kabi -
            # bir nechta premium yutuqlari QO'SHILIB BORMAYDI, faqat ENG
            # KATTA muddat qo'llanadi. Agar foydalanuvchida allaqachon
            # (xarid qilingan yoki oldingi yutuqdan) shu yutuqdan uzoqroq
            # muddatgacha amal qiladigan premium bo'lsa, yangi (kichikroq)
            # yutuq shunchaki bekor bo'ladi va premium_until o'zgarmaydi.
            duration_key = _BARABAN_PREMIUM_KEY[prize_id]
            seconds = PREMIUM_DURATIONS.get(duration_key)
            if seconds:
                prow = conn.execute(
                ...
                now = int(time.time())
                current_until = (prow["premium_until"] or 0) if prow else 0
                candidate_until = now + seconds
                new_until = max(current_until, candidate_until)
                conn.execute(
                    "UPDATE users SET is_premium=1, premium_until=? WHERE user_id=?",
                    (new_until, user_id)
                )
        ...
        spin_row = conn.execute(
            "INSERT INTO baraban_spins (user_id, first_name, prize_id, prize_name, prize_icon, delivered) "
            "VALUES (?,?,?,?,?,?) RETURNING id",
            (user_id, first_name, prize_id, prize_name, chosen["icon"], delivered)
        ).fetchone()
        spin_id = spin_row["id"] if spin_row else None

        # UC, Elite Pass va chegirma yutuqlari uchun bir martalik voucher
        # yaratamiz - shu bilan foydalanuvchi tegishli bo'limda o'zi
        # "ishlatadi". Elite Pass = 325 UC qiymatiga teng bepul UC voucher.
        if is_uc and digits:
            conn.execute(
                "INSERT INTO baraban_vouchers (user_id, kind, value, spin_id) VALUES (?,?,?,?)",
                (user_id, "uc", int(digits), spin_id)
            )
            delivered = 1
        elif is_elite:
            conn.execute(
                "INSERT INTO baraban_vouchers (user_id, kind, value, spin_id) VALUES (?,?,?,?)",
                (user_id, "uc", _BARABAN_ELITE_UC_VALUE, spin_id)
            )
            delivered = 1
        elif is_discount and digits:
            conn.execute(
                "INSERT INTO baraban_vouchers (user_id, kind, value, spin_id) VALUES (?,?,?,?)",
                (user_id, "discount", int(digits), spin_id)
            )
            delivered = 1

        if delivered:
            conn.execute(
                "UPDATE baraban_spins SET delivered=1 WHERE id=?", (spin_id,)
            )

        conn.commit()

        left = conn.execute(
            "SELECT tickets, balance FROM users WHERE user_id=?", (user_id,)
        ).fetchone()
        conn.close()
        return {
            "ok": True,
            "prize": dict(chosen),
            "tickets_left": left["tickets"] if left else 0,
            "balance": left["balance"] if left else 0,
        }
    except Exception:
        conn.rollback()
        conn.close()
