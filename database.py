...
def spin_baraban(user_id: int, first_name: str = None) -> dict:
    ...
    try:
        ...
        # Pul mukofoti bo'lsa (nomi so'm bilan tugasa), balansga avtomatik qo'shiladi
        if "so'm" in chosen["name"]:
        ...
        conn.execute(
            "INSERT INTO baraban_spins (user_id, first_name, prize_id, prize_name, prize_icon) "
            "VALUES (?,?,?,?,?)",
            (user_id, first_name, chosen["id"], chosen["name"], chosen["icon"])
        )
