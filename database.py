...
def spin_baraban(user_id: int, first_name: str = None) -> dict:
    ...
    'weight' ustuniga proporsional ehtimollik bilan tanlanadi. Agar
    yutuq turi balansga tegishli bo'lsa (masalan 'so'm'), shu yerning
    o'zida foydalanuvchi balansiga qo'shib qo'yiladi."""
    conn = get_conn()
    try:
        row = conn.execute(
        ...
        # Pul mukofoti bo'lsa (nomi so'm bilan tugasa), balansga avtomatik
        # qo'shiladi va shu zahoti "berildi" deb belgilanadi. Boshqa turdagi
        # sovg'alar (UC, Premium, Elite Pass, chegirma) admin panelidagi
        # "Baraban yutuqlari" bo'limida qo'lda "berildi" deb belgilanguncha
        # kutilayotgan hisoblanadi.
        is_cash = "so'm" in chosen["name"]
        if is_cash:
            digits = "".join(ch for ch in chosen["name"] if ch.isdigit())
            if digits:
                conn.execute(
                    "UPDATE users SET balance = balance + ? WHERE user_id=?",
                    (int(digits), user_id)
        ...
        conn.execute(
            ...
            "VALUES (?,?,?,?,?,?)",
            (user_id, first_name, chosen["id"], chosen["name"], chosen["icon"], 1 if is_cash else 0)
        )
