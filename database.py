...
def spin_baraban(user_id: int, first_name: str = None) -> dict:
    ...
    qarab AVTOMATIK qo'llaniladi:
      ...
      - "Elite Pass" -> hozircha ilovada bunga mos funksiya yo'q, shuning
                     uchun admin panelidan qo'lda "berildi" deb belgilanadi
    """
    conn = get_conn()
    try:
        row = conn.execute(
        ...
        # UC va chegirma yutuqlari uchun bir martalik voucher yaratamiz -
        # shu bilan foydalanuvchi tegishli bo'limda o'zi "ishlatadi".
        if is_uc and digits:
            conn.execute(
        ...
        left = conn.execute("SELECT tickets FROM users WHERE user_id=?", (user_id,)).fetchone()
        conn.close()
        return {
            "ok": True,
            "prize": dict(chosen),
