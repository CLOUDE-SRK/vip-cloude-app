...
class _ConnWrapper:
    ...
    def execute(self, sql, params=()):
        ...
        # SQLite'dagi '?' placeholder'lari Postgres'da '%s' bo'ladi.
        # Loyihadagi SQL satrlarining hech birida haqiqiy qiymat sifatida
        # '?' belgisi ishlatilmaydi, shuning uchun bu almashtirish xavfsiz.
...
def has_screenshot_today(user_id: int) -> bool:
    ...
    row = conn.execute(
        ...
        WHERE user_id=? AND method='screenshot'
          AND (to_timestamp(created_at) AT TIME ZONE 'UTC')::date = (now() AT TIME ZONE 'UTC')::date
        LIMIT 1
        """,
        (user_id,)
    ).fetchone()
