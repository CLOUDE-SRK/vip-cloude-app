...
def add_earn_tap(user_id: int, amount: int, cap: int = 500) -> dict:
    ...
    XAVFSIZLIK: bu funksiya avval kunlik jami summani o'qiydi, keyin
    yozadi — ikkisi orasida "bo'shliq" bor edi. Agar foydalanuvchi bir
    vaqtning o'zida bir nechta so'rov yuborsa (parallel/racing requests),
    har biri limitni "hali to'lmagan" deb ko'rishi va cap dan bir necha
    barobar ko'p yig'ib olishi mumkin edi. `BEGIN IMMEDIATE` shu
    foydalanuvchi uchun yozish qulfini SELECT dan OLDIN olib qo'yadi —
    shu bilan parallel so'rovlar navbat bilan, bittalab qayta ishlanadi.
    """
...
def cancel_uc_order(order_id: int):
    ...
    qaytaradi — avval bu funksiya faqat statusni o'zgartirar edi, pulni
    qaytarish chaqiruvchi kodga qoldirilgan edi, bu esa "pul qaytarilishi
    unutilib qolishi" yoki "ikki marta qaytarilishi" xavfini tug'dirardi."""
    conn = get_conn()
    try:
        conn.execute("BEGIN IMMEDIATE")
        row = conn.execute(
