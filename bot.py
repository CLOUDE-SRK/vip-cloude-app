...
# quyidagi handle_webapp_data, handle_photo va admin_confirm handlerlaridan
# OLDIN turishi shart — aks holda masalan admin raqam yuborsa (masalan
# narx e'lon qilsa), admin_confirm handleri uni "to'lov tasdiqlash" deb
# tushunib qolar edi.
@dp.message_handler(
...
async def announce_receive_button_url(message: types.Message):
    ...
    # Boshqa har qanday havola (t.me kanal, boshqa sayt va h.k.) uchun
    # esa oddiy url tugma qoldiriladi, chunki web_app faqat o'z
    # domenimiz uchun ishlaydi.
    if url.rstrip("/").startswith(WEBAPP_URL.rstrip("/")):
