...
# Server (Render) har doim UTC vaqtda ishlaydi, lekin biz Toshkent
# vaqtida (UTC+5, yil bo'yi o'zgarmaydi) ko'rsatishimiz kerak.
...
MAX_RECEIPT_BYTES = 8 * 1024 * 1024  # 8 MB — Telegram sendPhoto limitidan biroz past
...
VIP_PACKAGES = {
    ...
    "3oy":      {"name": "3 oylik",    "base_price": 150000},
    "1yil":     {"name": "1 yillik",   "base_price": 500000},
...
def _format_topup_history_message(row, status_label: str) -> str:
    """Admin panelidagi 'Pul o'tkazmalar' tarixi kartochkasi bilan bir xil
    ko'rinishdagi xabar — bot.py dagi format_topup_history_message bilan
    ATAYLAB bir xil, shunda panel orqali tasdiqlansa ham, botdagi xabar
    ...
    dt = (datetime.fromtimestamp(row["created_at"], tz=timezone.utc) if row["created_at"] else datetime.now(timezone.utc)).astimezone(UZ_TZ)

    return (
        f"{status_emoji} <b>O'tkazma #{row['id']}</b> · {dt.strftime('%d.%m %H:%M')}\n"
        f"Holat: <b>{status_label}</b>\n\n"
        f"👤 Foydalanuvchi: {_esc(name)}\n"
        f"🆔 TG ID: <code>{row['user_id']}</code>\n"
        f"💵 Summa: <b>{row['amount']:,} so'm</b>\n"
        f"🏦 Usul: {_esc(row['method'])}\n\n"
        f"<i>(Boshqaruv paneli orqali)</i>"
    )


async def _notify_admin_bot_topup(row, status_label: str):
    """Admin panelidan tasdiqlangan/bekor qilingan topup haqida ADMIN_ID ga
    Telegram orqali xabar yuboradi — shu bilan bot va panel tarixi
    sinxron ko'rinadi."""
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            await client.post(
                ...
                json={
                    "chat_id": ADMIN_ID,
                    "text": _format_topup_history_message(row, status_label),
                    "parse_mode": "HTML",
                },
            )
    except Exception as e:
        logging.warning(f"[TOPUP] Admin botga xabar yuborilmadi: {e}")
...
async def get_balance(request: Request):
    ...
    return {
        ...
        # Premium tugagach, oldin o'rnatilgan avatar rasmi ham
        # ko'rinmasligi kerak — faqat premium hali aktiv bo'lsa qaytariladi.
        "avatar_url": (profile["avatar_url"] if profile and is_premium else None),
        "equipped_frame": profile["equipped_frame"] if profile else None,
        "equipped_color": profile["equipped_color"] if profile else None
    }
...
@app.get("/skins/catalog")
async def skins_catalog(request: Request):
    """'Skin do'koni' uchun to'liq katalog - ramkalar, ism effektlari,
    ularning narxi, va joriy foydalanuvchi uchun sotib olingan/kiyilgan
    holati."""
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")
    return db.get_cosmetic_catalog(user["id"])


class SkinItemRequest(BaseModel):
    item_id: str


@app.post("/skins/buy")
async def skins_buy(request: Request, body: SkinItemRequest):
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")
    ...
    result = db.buy_cosmetic(user["id"], body.item_id)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result
...
async def skins_equip(request: Request, body: SkinItemRequest):
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")

    result = db.equip_cosmetic(user["id"], body.item_id)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result
...
class SkinUnequipRequest(BaseModel):
    category: str   # 'frame' | 'color'


@app.post("/skins/unequip")
async def skins_unequip(request: Request, body: SkinUnequipRequest):
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")

    result = db.unequip_cosmetic_category(user["id"], body.category)
    if "error" in result:
        raise HTTPException(status_code=400, detail=result["error"])
    return result
...
# Chipta paketlari - index.html dagi TICKET_PACKAGES bilan bir xil
# bo'lishi SHART (frontendda narx faqat ko'rsatish uchun, haqiqiy
# tekshiruv har doim shu yerda, serverda amalga oshadi).
TICKET_PACKAGES = {
    1:  2000,
    5:  8000,
    10: 15000,
...
@app.get("/baraban/state")
async def baraban_state(request: Request):
    """'Omad barabani' oynasi ochilganda kerak bo'ladigan hamma narsa:
    foydalanuvchining chipta soni, sovg'alar katalogi va so'nggi
    g'oliblar ro'yxati."""
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
    ...
    return {
        "tickets": db.get_tickets(user["id"]),
        "prizes": db.get_baraban_catalog(),
        "winners": db.get_baraban_recent_winners(10),
        "vouchers": db.get_active_baraban_vouchers(user["id"]),
    }


@app.post("/baraban/spin")
async def baraban_spin(request: Request):
    """Barabanni bir marta aylantiradi (1 chipta sarflanadi)."""
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")

    result = db.spin_baraban(user["id"], user.get("first_name"))
    if result.get("error") == "no_tickets":
        raise HTTPException(status_code=400, detail="Chiptalar yetarli emas")
    if result.get("error") == "no_prizes":
        raise HTTPException(status_code=500, detail="Baraban katalogi bo'sh")

    # Foydalanuvchiga yutuq haqida Telegram orqali darhol xabar beramiz -
    # ayniqsa qimmatli sovg'alar (UC, chegirma, premium, Elite Pass) uchun
    # bu muhim, chunki ular avtomatik/qo'lda qayerdan olinishini eslatadi.
    try:
        ...
        note = None
        if "so'm" in prize_name:
            note = "✅ Balansingizga avtomatik qo'shildi."
        elif prize_id.startswith("baraban_uc"):
            note = "🎟️ Bir martalik voucher yaratildi — \"UC sotib olish\" bo'limida shu miqdorni 0 so'mga oling."
        elif prize_id.startswith("baraban_disc"):
            note = "🏷️ Bir martalik chegirma voucheri yaratildi — VIP/Premium xaridida avtomatik qo'llanadi."
        elif prize_id in db._BARABAN_PREMIUM_KEY:
            note = "💎 Premium profilingizga avtomatik qo'shildi."
        elif prize_id == "baraban_elite":
            note = "🎫 Admin tez orada Elite Pass'ni hisobingizga qo'lda beradi."
            ...
            async with httpx.AsyncClient(timeout=5.0) as client:
                await client.post(
                    ...
                    json={
                        ...
                        "text": (
                            f"🎉 <b>Omad barabani</b>\n\n"
                            f"Siz yutdingiz: <b>{prize_icon} {prize_name}</b>\n\n"
                            f"{note}"
                        ),
                        "parse_mode": "HTML",
                    }
        ...
        # Bildirishnoma yuborishda xato bo'lsa ham, foydalanuvchi natijani
        # webapp ichida ko'rishda davom etadi - shuning uchun bu joyda
        # xatoni yutib yuboramiz.
        pass

    return result


@app.get("/baraban/vouchers")
async def baraban_vouchers(request: Request):
    """Foydalanuvchining hali ishlatilmagan barcha baraban voucherlari
    (UC sotib olish va VIP xarid bo'limlarida ko'rsatish uchun)."""
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")

    return {"vouchers": db.get_active_baraban_vouchers(user["id"])}


class BuyTicketsBody(BaseModel):
    count: int


@app.post("/baraban/buy_tickets")
async def baraban_buy_tickets(request: Request, body: BuyTicketsBody):
    """Balansdan chipta sotib oladi. Narx TICKET_PACKAGES orqali serverda
    qat'iy tekshiriladi - clientdan kelgan narxga ishonilmaydi."""
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")

    price = TICKET_PACKAGES.get(body.count)
    if price is None:
        raise HTTPException(status_code=400, detail="Noto'g'ri paket")

    result = db.buy_tickets(user["id"], body.count, price)
...
async def get_leaderboard():
    ...
    for row in rows:
        ...
        result.append({
            ...
            "initials": initials,
            "refs": row["refs"],
            # Premium tugagan foydalanuvchining eski avatar rasmi Top
            # ro'yxatida ham endi ko'rsatilmaydi.
            "avatar_url": (row["avatar_url"] if is_premium else None),
            "is_premium": is_premium
        })
...
class VipOrderRequest(BaseModel):
    ...
    voucher_id: int = None   # Baraban orqali yutilgan chegirma voucheri ID'si


# "Premium sotib olish" bo'limi uchun paketlar — VIP_PACKAGES bilan bir xil
# narxlarni ishlatadi, lekin bu butunlay boshqa funksiya: bu yerda foydalanuvchiga
# VIP kanal emas, balki profil imtiyozlari (is_premium=1) beriladi.
PREMIUM_PACKAGES = {
    k: VIP_PACKAGES[k] for k in ("15kun", "1oy", "3oy", "1yil") if k in VIP_PACKAGES
}


class TopupRequestBody(BaseModel):
    amount: int
    method: str            # "Uzcard", "Payme", "Binance"
    receipt_base64: str    # "data:image/jpeg;base64,...."


@app.post("/topup_request")
async def topup_request(request: Request, body: TopupRequestBody):
    """
    Balansni to'ldirish so'rovi. Foydalanuvchi WebApp ichida chek rasmini
    tanlaydi, u shu yerga (base64) yuboriladi. Rasm hech qayerda saqlanmaydi —
    to'g'ridan-to'g'ri Telegram Bot API orqali ADMIN_ID'ga (sizga) sendPhoto
    bilan jo'natiladi va topup_requests jadvaliga 'pending' holatda yoziladi.
    Admin (siz) odatdagidek raqam yozib tasdiqlaysiz (bot.py/admin_confirm).
    """
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
    ...
    method = (body.method or "").strip() or "noma'lum"

    raw = body.receipt_base64 or ""
    if "," in raw:
        raw = raw.split(",", 1)[1]

    # XAVFSIZLIK: avatar yuklashda bo'lgani kabi, bu yerda ham hajmga yuqori
    # chegara YO'Q edi — kimdir juda katta base64 matn yuborib server
    # xotirasi/tarmog'ini behuda band qilishi mumkin edi. Base64 hajmi ≈
    # asl fayl hajmining 4/3 qismi bo'lgani uchun, decode qilishdan OLDIN
    # ham taxminiy tekshiruv qilamiz.
    if len(raw) > MAX_RECEIPT_BYTES * 4 // 3:
        raise HTTPException(status_code=400, detail="Chek rasmi hajmi juda katta (8MB dan oshmasligi kerak)")

    try:
        photo_bytes = base64.b64decode(raw)
    except Exception:
        raise HTTPException(status_code=400, detail="Chek rasmi noto'g'ri formatda")

    if len(photo_bytes) < 100:
        raise HTTPException(status_code=400, detail="Chek rasmi bo'sh yoki noto'g'ri")
    if len(photo_bytes) > MAX_RECEIPT_BYTES:
        raise HTTPException(status_code=400, detail="Chek rasmi hajmi juda katta (8MB dan oshmasligi kerak)")
    ...
    # Avval bazaga yozamiz — shu topup_id orqali admin tugmasi (tasdiqlash/
    # bekor qilish) ANIQ shu so'rovga bog'lanadi. Bir nechta odam bir vaqtda
    # chek yuborsa ham, admin qaysi xabar kimga tegishli ekanini taxmin
    # qilishga majbur bo'lmaydi — har bir xabarning o'z tugmasi bor.
    topup_id = db.create_topup(user_id, amount, method, 0)

    caption = (
        f"💰 <b>Topup so'rov (chek bilan)</b>\n\n"
        f"👤 {user_link}\n"
    ...
    reply_markup = {
        "inline_keyboard": [
            [
                {"text": "✅ Tasdiqlash", "callback_data": f"topup_ok:{topup_id}"},
                {"text": "❌ Bekor qilish", "callback_data": f"topup_cancel:{topup_id}"},
            ]
        ]
    }
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            resp = await client.post(
                ...
                files={"photo": ("chek.jpg", photo_bytes, "image/jpeg")},
            )
            resp_data = resp.json()
            if not resp_data.get("ok"):
                raise RuntimeError(resp_data)
    except Exception as e:
        logging.error(f"[TOPUP] Admin ga chek yuborishda XATO: {e}")
        raise HTTPException(status_code=502, detail="Chekni yuborishda xatolik. Qayta urining.")
...
class BuyPremiumRequest(BaseModel):
    duration_key: str   # masalan: "15kun", "1oy", "3oy", "1yil"
    voucher_id: int = None   # Baraban orqali yutilgan chegirma voucheri ID'si


@app.post("/buy_premium")
async def buy_premium(request: Request, body: BuyPremiumRequest):
    init_data = request.headers.get("X-Init-Data", "")
    user = verify_init_data(init_data)
    if not user:
        raise HTTPException(status_code=401, detail="Invalid initData")
    ...
    pkg = PREMIUM_PACKAGES.get(body.duration_key)
    if not pkg:
        raise HTTPException(status_code=400, detail="Noto'g'ri paket")
    ...
    if body.voucher_id is not None:
        voucher = db.get_baraban_voucher(body.voucher_id, user_id, kind="discount")
        if not voucher:
            raise HTTPException(status_code=400, detail="Voucher topilmadi yoki allaqachon ishlatilgan")
        discount = max(discount, voucher["value"])
    ...
    if voucher:
        db.mark_baraban_voucher_used(voucher["id"])

    new_until = db.gift_premium(user_id, body.duration_key)
    if new_until is None:
        # Amaliy jihatdan bo'lmasligi kerak (kalit yuqorida tekshirilgan),
        # lekin xato bo'lsa pulni qaytaramiz.
        db.add_balance(user_id, final_price)
        raise HTTPException(status_code=400, detail="Noto'g'ri paket")

    db.create_vip_order(user_id, f"Premium ({pkg['name']})", final_price)
    ...
    return {"ok": True, "price": final_price, "premium_until": new_until}
...
async def buy_vip(request: Request, body: VipOrderRequest):
    ...
    # "Omad barabani"dan yutilgan chegirma voucheri bo'lsa - eng katta
    # chegirmani qo'llaymiz (referal yoki voucher, qaysi biri katta bo'lsa).
    voucher = None
    if body.voucher_id is not None:
        voucher = db.get_baraban_voucher(body.voucher_id, user_id, kind="discount")
        if not voucher:
...
class UcOrderRequest(BaseModel):
    player_id: str
    package_index: int = -1   # UC_PACKAGES ro'yxatidagi indeks (0-7)
    voucher_id: int = None    # Baraban orqali yutilgan bepul UC voucher ID'si
...
async def uc_order(request: Request, body: UcOrderRequest):
    ...
    voucher = None
    if body.voucher_id is not None:
        # "Omad barabani"da yutilgan bepul UC voucherini ishlatish - narx 0,
        # bitta paketga bog'liq emas, aynan yutilgan UC miqdori beriladi.
        voucher = db.get_baraban_voucher(body.voucher_id, user_id, kind="uc")
        if not voucher:
            raise HTTPException(status_code=400, detail="Voucher topilmadi yoki allaqachon ishlatilgan")
        uc_amount = voucher["value"]
        price = 0
    else:
    ...
    try:
        ...
        admin_text = (
            ...
            f"Pastdagi tugmalar orqali shu yerdan yoki admin panelda tasdiqlang."
        )
        # Botning o'zida ✅/❌ tugmalari orqali ham tasdiqlash/bekor qilish
        # imkoni bo'lishi uchun — endi faqat admin panelga kirish shart emas.
        reply_markup = {
            "inline_keyboard": [
                [
                    {"text": "✅ Tasdiqlash", "callback_data": f"ucorder_ok:{order_id}"},
                    {"text": "❌ Bekor qilish", "callback_data": f"ucorder_cancel:{order_id}"},
                ]
            ]
        }
        async with httpx.AsyncClient(timeout=5.0) as client:
...
async def admin_approve_order(order_id: int, request: Request):
    ...
    if not ok:
        raise HTTPException(status_code=400, detail="Bu buyurtma topilmadi yoki allaqachon ko'rib chiqilgan")
    return {"ok": True}
...
# ── ADMIN PANEL: 1b) PUL O'TKAZMALAR (balans to'ldirish so'rovlari) ────
# Bu bo'lim "Buyurtmalar" (UC sotib olish) dan farqli — bu yerda
# foydalanuvchi balansini to'ldirish uchun yuborgan chek/so'rovlar
# ro'yxati va ularning tarixi (tasdiqlangan/bekor qilingan) ko'rsatiladi.
# Botdagi ✅/❌ tugmalari orqali tasdiqlangan so'rovlar ham shu yerda
# darhol yangilangan holatda ko'rinadi (bitta umumiy baza).
@app.get("/admin/topups")
async def admin_list_topups(request: Request, status: str = "pending"):
    require_admin(request)

    if status == "all":
        rows = db.get_recent_topups(limit=50)
    else:
...
@app.post("/admin/topups/{topup_id}/approve")
async def admin_approve_topup(topup_id: int, request: Request):
    require_admin(request)
    ...
    updated = db.get_topup(topup_id)
    await _notify_admin_bot_topup(updated, "Tasdiqlangan")
    return {"ok": True}


@app.post("/admin/topups/{topup_id}/cancel")
async def admin_cancel_topup(topup_id: int, request: Request):
    require_admin(request)

    result = db.reject_topup(topup_id)
    if not result:
        raise HTTPException(status_code=400, detail="So'rov topilmadi yoki allaqachon ko'rib chiqilgan")

    updated = db.get_topup(topup_id)
    await _notify_admin_bot_topup(updated, "Rad etilgan")
    return {"ok": True}


# ── ADMIN PANEL: Baraban yutuqlari (UC/Premium/Elite Pass/chegirma) ────
@app.get("/admin/baraban/deliveries")
async def admin_baraban_deliveries(request: Request, status: str = "pending"):
    require_admin(request)
    pending_only = status != "all"
    rows = db.get_baraban_deliveries(pending_only=pending_only, limit=200)
    result = []
    for r in rows:
        name = r["first_name"] or r["username"] or "Foydalanuvchi"
        result.append({
...
@app.post("/admin/baraban/deliveries/{spin_id}/deliver")
async def admin_baraban_mark_delivered(spin_id: int, request: Request):
    require_admin(request)
    ok = db.mark_baraban_delivered(spin_id)
    if not ok:
...
async def admin_list_verify_requests(request: Request):
    # DIQQAT: bu endpoint faqat "pending" (hali ko'rib chiqilmagan) so'rovlarni
    # qaytaradi — bazada tasdiqlangan/rad etilganlar tarixini olish funksiyasi
    # yo'q, shuning uchun oldingi "status" parametri hech qanday ta'sir
    # qilmasdan doim e'tiborsiz qoldirilar edi. Chalg'itmaslik uchun olib
    # tashlandi.
    require_admin(request)
...
async def admin_approve_verify_request(request_id: int, request: Request):
    ...
    # DIQQAT: admin panelda qilingan bu amal haqida foydalanuvchiga endi
    # Telegram xabari YUBORILMAYDI (ataylab) — foydalanuvchi natijani
    # ilova ichida (balans/vazifalar bo'limida) o'zi ko'radi.
    return {"ok": True}
...
async def admin_add_balance(user_id: int, request: Request, body: AddBalanceBody):
    ...
    # DIQQAT: admin panelda qilingan bu amal haqida foydalanuvchiga endi
    # Telegram xabari YUBORILMAYDI (ataylab) — yangilangan balansni
    # foydalanuvchi ilova ichida o'zi ko'radi.
    return {"ok": True, "balance": db.get_balance(user_id)}


class AddTicketsBody(BaseModel):
    amount: int


@app.post("/admin/profile/{user_id}/add_tickets")
async def admin_add_tickets(user_id: int, request: Request, body: AddTicketsBody):
    """'Omad barabani' uchun chipta berish. HOZIRCHA: chiptalarni sotib
    olish tizimi hali frontendda yo'q, shuning uchun admin panel orqali
    qo'lda beriladi - keyinchalik xarid oqimi qo'shilganda bu funksiya
    o'sha oqim ichidan ham chaqiriladi."""
    require_admin(request)
    row = db.get_profile_admin(user_id)
    if not row:
        raise HTTPException(status_code=404, detail="Foydalanuvchi topilmadi")
    new_total = db.add_tickets(user_id, body.amount)
    return {"ok": True, "tickets": new_total}
...
async def admin_gift_premium(user_id: int, request: Request, body: GiftPremiumBody):
    ...
    # DIQQAT: admin panelda qilingan bu amal haqida foydalanuvchiga endi
    # Telegram xabari YUBORILMAYDI (ataylab) — Premium holatini
    # foydalanuvchi ilova ichida (profil bo'limida) o'zi ko'radi.
