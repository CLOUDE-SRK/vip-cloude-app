...
# DIQQAT: bu qism ma'lumotlar bazasi muammosidan ALOHIDA masala.
# Baza endi Supabase'da (Render'dan mustaqil) saqlanadi, lekin bu YUKLANGAN
# RASMLARGA (profil avatarlari, vazifa logotiplari) taalluqli emas — ular
# hamon shu konteynerning mahalliy diskiga yoziladi. Render'ning bepul
# tarifida fayl tizimi "ephemeral" bo'lgani uchun bu rasmlar HAR DEPLOY/
# RESTART'DA HALI HAM O'CHIB KETADI. Buni tuzatish uchun keyinchalik yoki
# (a) Render'da pullik tarifga o'tib persistent disk ulash, yoki (b) rasmlarni
# ham Supabase Storage (bepul) kabi tashqi xizmatga ko'chirish kerak bo'ladi.
_UPLOADS_ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "uploads")

# Profil rasmlari shu papkaga saqlanadi va /uploads/... orqali hammaga ochiq bo'ladi
UPLOAD_DIR = os.path.join(_UPLOADS_ROOT, "avatars")
os.makedirs(UPLOAD_DIR, exist_ok=True)

# Vazifa logotiplari shu papkaga saqlanadi
TASK_ICON_DIR = os.path.join(_UPLOADS_ROOT, "task_icons")
os.makedirs(TASK_ICON_DIR, exist_ok=True)
...
async def set_profile_avatar(request: Request, file: UploadFile = File(...)):
    ...
    if old_url and old_url.startswith("/uploads/avatars/"):
        old_path = os.path.join(os.path.dirname(UPLOAD_DIR), old_url.lstrip("/").split("/", 1)[1])
        try:
...
async def earn_tap(request: Request, body: EarnTapRequest):
    ...
    result = db.add_earn_tap(user_id, amount, cap=100)

    # VAQTINCHALIK DIAGNOSTIKA — 2x boost muammosini topish uchun.
    # Muammo topilgach, bu qatorni olib tashlaymiz.
    logging.warning(
        f"[EARN_TAP DEBUG] user_id={user_id} yuborilgan_amount={amount} "
        f"premium={result['premium']} cap={result['cap']} "
        f"qo'shilgan={result['added']} yangi_balans={result['balance']} "
        f"bugungi_jami={result['earn_today']}"
    )
...
async def admin_upload_task_icon(request: Request, file: UploadFile = File(...)):
    ...
    filepath = os.path.join(TASK_ICON_DIR, filename)
    with open(filepath, "wb") as f:
        f.write(contents)

    icon_url = f"/uploads/task_icons/{filename}"
