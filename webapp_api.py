...
async def baraban_spin(request: Request):
    ...
    # DIQQAT: bu yerda foydalanuvchiga Telegram orqali alohida xabar
    # yuborilmaydi - yutuq natijasi faqat webapp ichidagi natija oynasida
    # ko'rsatiladi. (Oldin har bir aylantirishda botdan xabar kelardi,
    # bu olib tashlandi.)
    return result
...
async def buy_premium(request: Request, body: BuyPremiumRequest):
    ...
    # DIQQAT: mijoz qaysi voucher_id yuborganidan qat'i nazar, server
    # foydalanuvchining barcha faol chegirma voucherlari orasidan ENG
    # KATTASINI o'zi tanlaydi va shuni qo'llaydi - shu bilan bir nechta
    # chegirma yig'ilib qolsa ham har doim faqat eng balandi ishlaydi,
    # pastroq chegirmalar hech qachon uni bekor qilmaydi/ustidan yozmaydi.
    voucher = db.get_best_discount_voucher(user_id)
    if voucher:
...
async def buy_vip(request: Request, body: VipOrderRequest):
    ...
    # "Omad barabani"dan yutilgan chegirma voucherlari bo'lsa - mijoz
    # nimani yuborganidan qat'i nazar, server foydalanuvchining barcha
    # faol voucherlari orasidan ENG KATTASINI tanlaydi va qo'llaydi
    # (referal yoki voucher, qaysi biri katta bo'lsa). Shu bilan pastroq
    # chegirma voucherlari hech qachon avvalgi baland chegirmani bekor
    # qilib yubormaydi - doim eng balandi ustuvor bo'ladi.
    voucher = db.get_best_discount_voucher(user_id)
    if voucher:
        discount = max(discount, voucher["value"])
